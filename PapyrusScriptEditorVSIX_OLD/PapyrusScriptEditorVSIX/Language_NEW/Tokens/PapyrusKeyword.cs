...
namespace Papyrus.Language_NEW.Tokens {
    public class PapyrusKeyword : IPapyrusToken, ISyntaxColorableToken {
        ...
        public bool ConvertToText(StringBuilder stringBuilder, TextFormatInfo textFormatInfo) {
            stringBuilder.Append(textFormatInfo.FormatToken(Name));
            return true;
        }
    ...
    internal sealed class PapyrusKeywordParser : ITokenParser {
        public bool TryParse(string sourceTextSpan, TokenScanner scanner, IEnumerable<IPapyrusToken> previousTokens, out IPapyrusToken token) {
            if (scanner.CurrentState == TokenScannerState.Default) {
                int nextDelim = PapyrusDelimiter.FindNext(sourceTextSpan, 0);
                PapyrusKeyword keyword = PapyrusKeyword.FromName(sourceTextSpan.Remove(nextDelim));
                if (keyword != null) {
                    token = keyword;
                    return true;
                }
