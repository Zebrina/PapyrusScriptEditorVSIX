...
namespace Papyrus.Language_NEW.Tokens {
    public class PapyrusNumericLiteral : IPapyrusToken, ISyntaxColorableToken {
        ...
        public bool IsEqualToToken(IPapyrusToken other) {
            return other is PapyrusNumericLiteral && Value.Equals(((PapyrusStringLiteral)other).Value, StringComparison.OrdinalIgnoreCase);
        }
    ...
    internal sealed class PapyrusNumericLiteralParser : ITokenParser {
        public bool TryParse(string sourceTextSpan, TokenScanner scanner, IEnumerable<IPapyrusToken> previousTokens, out IPapyrusToken token) {
            if (scanner.CurrentState == TokenScannerState.Default) {
                PapyrusNumericLiteral numericLiteral = PapyrusNumericLiteral.Parse(sourceTextSpan);
