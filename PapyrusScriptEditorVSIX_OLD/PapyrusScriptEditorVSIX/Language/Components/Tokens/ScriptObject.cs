...
namespace Papyrus.Language.Components.Tokens {
    public interface IScriptObjectComponent {
        bool IsSubComponent { get; }
        int NumTokens();
        bool TryParse(IEnumerable<Token> tokens);
    }
    ...
    public sealed partial class ScriptObject {
        ...
        private class ScriptObjectManager : IScriptObjectManager, IDictionary<string, ScriptObject>, IReadOnlyDictionary<string, ScriptObject>, IEnumerable<ScriptObject>, IEnumerable {
            ...
            public bool TryGetValue(string key, out ScriptObject value) {
                ...
                if (PapyrusEditor.ActiveGame != null) {
                    string filePath = PapyrusEditor.FindSourceFile(key);
    ...
    internal sealed class ScriptObjectParser : TokenParser {
        public override bool TryParse(string sourceTextSpan, TokenScanner scanner, IEnumerable<Token> previousTokens, out Token token) {
            if (scanner.TopLevelState == ScannerStates.GlobalScope) {
                // Treat token succeding 'Scriptname' as Identifier instead of ScriptObject.
                Token lastToken = previousTokens.LastOrDefault();
                if (lastToken == null || lastToken != Keyword.Scriptname) {
